...
function Sidebar() {
 ...
 const logoutHandler=()=>{
   ...
   let option1=[
      ...
      {name:"About",color:'text-red-500',size:'w-10'},
      {name:"Login",to:'/login'},
      {name:"Contact Us",color:'text-red-500',size:'w-10'},
   ]
...
<nav id="default-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 py-4 overflow-y-auto bg-slate-800  ">
...

      {
         ...
         <li>
      {op.name === "Logout" ? (
         <div  onClick={logoutHandler} class="flex items-center p-2 cursor-pointer text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
        <button>
          {op.svg}
